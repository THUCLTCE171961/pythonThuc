...
import sys
import io
from winforms_sender import receive_gesture_name, start_camera_server, send_frame_to_winforms, send_status_to_winforms, start_status_server


sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8')
# === CONFIG ===
CAPTURE_CSV = 'gesture_motion_capture.csv'
BUFFER_SIZE = 60
SMOOTHING_WINDOW = 3
MIN_FRAMES = 12
MIN_CONFIDENCE = 0.7
...
def main():
    ...
    pose_label = receive_gesture_name()
    if not pose_label:
    ...
    try:
        while cap.isOpened():
            ...
            send_status_to_winforms(status_conn, state, pose_label, saved_count)  # <--- Thêm dòng này
