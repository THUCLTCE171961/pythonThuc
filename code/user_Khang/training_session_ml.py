...
# ✅ NEW: Multilingual status messages for training
STATUS_MESSAGES = {
    "CORRECT": {
        "en": "CORRECT",
        "vi": "ĐÚNG"
...
def send_status_to_winforms(conn, result, pose, correct, wrong, acc, message_key, *args):
    ...
    try:
        ...
        # New format with bilingual reasons
...
#                            cam_conn=None,
#                            status_conn=None):
#     """Run training session with ML models and send data to WinForms"""
...
def run_ml_training_session(camera_index: int = 0, pose_label: str = "", cam_conn=None, status_conn=None):
    """
    Training session for ML model. 
    Sends frame data and training status to WinForms via sockets (cam_conn, status_conn).
    """
    # Load ML models and templates
    try:
        svm_model, label_encoder, scaler, static_dynamic_data = load_models()
    ...
    try:
        while True:
                    ...
                    if motion_features is None:
                        ...
                        if status_conn:
                            send_status_to_winforms(
                                status_conn, "WRONG", target_gesture,
                                stats.correct, stats.wrong, stats.accuracy() * 100,
                                "Failed to process motion"
                            )
                    else:
                        # ML Evaluation strict mode
                        success, reason_code, reason_msg = evaluate_with_ml(
                            recorded_left_states, recorded_right_states, 
                            motion_features, target_gesture, 
                            svm_model, label_encoder, scaler, static_dynamic_data,
            ...
            # COULD: cv2.imshow() for debug purpose, NOT needed for WinForms mode.
            # cv2.imshow("ML Training Session", frame)
            # key = cv2.waitKey(1) & 0xFF
            # if key == ord('q'):
            #     break
...
def main():
    ...
    pose_label = receive_pose_name()  # Nhận tên pose từ WinForms
    if not pose_label:
        print("[WARN] No pose_label received. Exiting.")
        return 1

    cam_conn = start_camera_server()  # Kênh gửi frame
    status_conn = start_status_server()  # Kênh gửi trạng thái
    try:
